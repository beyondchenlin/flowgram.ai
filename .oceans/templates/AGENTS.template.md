...
- 当前仓库路径：`/Users/wangyanxiang/Documents/code/flowgram`
- FlowGram 是 Rush 管理的 TypeScript/React monorepo，生产库位于 `packages/*`，示例和文档位于 `apps/*`，共享配置和 Rush 工具位于 `config/` 与 `common/`，E2E 位于 `e2e/*`。
- 新代码应落在最接近的既有 package；只有模块确实需要独立版本和发布周期时，才新增 Rush project。
...
- 能用命令确定检查结果的内容，优先放到 `scripts/`；macOS/Linux 的本地钩子启用、修复和初始化入口是 `scripts/agent-bootstrap.sh`，Windows 或已安装 PowerShell Core 时可使用 `scripts/agent-bootstrap.ps1`。
- 提交前基础门禁与专项验证在 macOS/Linux 默认使用 `scripts/agent-verify.sh`；Windows 或已安装 PowerShell Core 时可使用 `scripts/agent-verify.ps1`。
- 必须每次执行、不能依赖模型自觉的约束，优先放到 `.githooks/`；hooks 的安装与修复由 `scripts/agent-bootstrap.sh` 或 `scripts/agent-bootstrap.ps1` 负责，实际校验逻辑由 `scripts/agent-verify.sh` / `scripts/agent-verify.ps1` 承接。
...
- `main` 是默认基线；除只读任务外，改动必须在 `codex/<任务名>` linked worktree 分支中完成，除非用户明确说明不使用 worktree。
- 提交必须原子化，提交说明使用中文并带类型前缀，例如 `docs: 更新 agent 规则索引`。
- 每次修复或改动完成并通过验证后，必须自动安全提交、合并并推送到本地 `main` 与远程 `main`；不得夹带无关提交、覆盖未提交改动或使用强制推送。
...
- macOS 默认不能直接执行 `.ps1`；只有安装 PowerShell Core 后，才能用 `pwsh -NoProfile -File scripts/agent-verify.ps1` 或 `pwsh -NoProfile -File scripts/agent-bootstrap.ps1`。
- macOS/Linux 提交前默认运行 `scripts/agent-verify.sh` 完成基础门禁；本地 hooks 未启用或需要修复时，运行 `scripts/agent-bootstrap.sh`。
- Windows 或已安装 PowerShell Core 时，可运行 `scripts/agent-verify.ps1` / `scripts/agent-bootstrap.ps1`；专项验证自动同步到 `main` 时，PowerShell 使用 `-AllowDevBranch`，Shell 使用 `--allow-dev-branch`。
- FlowGram 代码验证优先使用 Rush 现有命令：`rush build`、`rush lint`、`rush ts-check`、`rush test`、`rush e2e:test`；局部包可在包目录使用 `rushx build`、`rushx test`、`rushx ts-check`。
