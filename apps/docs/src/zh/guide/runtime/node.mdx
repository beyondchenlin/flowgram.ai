...
Debug Output 节点是 FlowGram Runtime 的内置节点，用于在工作流执行过程中观察已经解析的值。它会将所有输入原样透传为输出，因此下游节点可以继续使用这些值，同时运行时快照会记录实际观察到的输入和输出。

#### 配置选项

| 选项 | 类型 | 必填 | 描述 |
|------|------|------|------|
| inputs | JSONSchema | 是 | 定义需要观察的值结构 |
| inputsValues | `Record<string, ValueSchema>` | 是 | 定义每个观察值的来源 |
| outputs | JSONSchema | 是 | 定义透传输出结构 |
...
构建强类型工作流 schema 或 UI 集成时，可以从 `@flowgram.ai/runtime-interface` 导入 `FlowGramNode.DebugOutput`、`DebugOutputNodeSchema` 和 `DebugOutputNodeData`。
