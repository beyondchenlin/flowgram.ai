...
> Future support will include Intent and Batch nodes
...
The Debug Output node is a built-in runtime node for inspecting resolved values during workflow execution. It passes every resolved input through unchanged as an output, so downstream nodes can keep using the same values while runtime snapshots show what was observed.

#### Configuration Options

| Option | Type | Required | Description |
|------|------|------|------|
| inputs | JSONSchema | Yes | Defines the values to observe |
| inputsValues | `Record<string, ValueSchema>` | Yes | Defines where each observed value comes from |
| outputs | JSONSchema | Yes | Defines the pass-through output shape |
...
Import `FlowGramNode.DebugOutput`, `DebugOutputNodeSchema`, and `DebugOutputNodeData` from `@flowgram.ai/runtime-interface` when building typed workflow schemas or UI integrations.
