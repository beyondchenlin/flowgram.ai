...
## Versioned Save Helper

`saveWorkflowDocument` and `loadWorkflowDocument` are exported from `@flowgram.ai/fixed-layout-editor` for product integrations that need a stable document snapshot contract.
...
const storageKey = 'my-app.workflow';

const initialData = loadWorkflowDocument(storage, storageKey, fallbackData);
...
Committed saves validate all node forms and keep the previous snapshot when validation errors exist. For draft auto-save, opt in explicitly:
