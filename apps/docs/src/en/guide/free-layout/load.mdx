...
`saveWorkflowDocument` and `loadWorkflowDocument` are exported from `@flowgram.ai/free-layout-editor` for product integrations that need a stable document snapshot contract.
